...
3️⃣ Include synonyms, variants, acronyms, and abbreviations
4️⃣ Cover all relevant fields discovered from content:
...
5️⃣ Prefer:
   - Action phrases users search for (verbs + nouns)
...
`;
  ...
  // 🚨 Timeout wrapper (30s max per request)
  const timeout = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error("⏱️ Gemini timeout")), 30000)
  );
...
export async function generateSynonyms(): Promise<SynonymEntry[] | null> {
  try {
    ...
    for (const doc of wikiDocs) {
      if (size + doc.text.length > MAX_CHARS_PER_BATCH) {
        try {
